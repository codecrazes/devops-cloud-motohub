...
# Esse script faz o deploy do Container Group (ACI) com a aplicação e o banco MySQL
# O ACI é responsavel por gerenciar os containers Docker sem a necessidade de um cluster Kubernetes
# Ele usa o arquivo mysql_aci.yaml como template, substituindo as variáveis de ambiente necessárias para o deploy
# O Container Group vai rodar a aplicação e o MySQL juntos, usando o File Share paran persistência dos dados do MySQL
